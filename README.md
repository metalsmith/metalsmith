...
Metalsmith v3.0 supports node 4.x and up.
...
1. Run `node.js` with `--harmony_generators` flag set.
    1. Use `metalsmith@^2.0.3`
    2. `node --harmony_generators my_script.js`
    3. Using `package.json`: `"scripts": {"start": "node --harmony_generators my_script.js"}`. Run with `npm run`
2. Use `metalsmith@^2.0.3` and `npm install` [harmonize](https://www.npmjs.com/package/harmonize) and require before Metalsmith is used. e.g. `require("harmonize")(["harmony-generators"]);`
