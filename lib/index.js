...
/**
 * Enable debug logs programmatically for plugins that do not use the new `metalsmith.debug` but a separate `debug` instance.
 * This method produces dirty side-effects like overwriting `process.stderr.write` and `process.env` variables.
 * For proper usage in the API flow, this method needs to be executed *before* any metalsmith plugins *are required or imported*.
 * Below is an example of how to include both legacy and metalsmith.debug logs in both console or log file.
 * 
 * @example
```
const metalsmith = require('metalsmith')(__dirname)
const debugNS = 'metalsmith-*,@metalsmith/*'
const toLog = 'metalsmith.log' // set to false to disable

metalsmith.legacyDebug(debugNS, toLog)

const sitemap = require('metalsmith-sitemap')

metalsmith
  .env({ DEBUG: debugNs, DEBUG_LOG: toLog })
  .use(sitemap)
```
 * 
 * @param {string} namespaces debug namespaces
 * @param {boolean} logfile Whether to pipe output to a log file specified in `metalsmith.env('debug_log')`
 */
Metalsmith.prototype.legacyDebug = function (namespaces, logfile) {
  process.env.DEBUG = namespaces
  if (logfile) {
    process.env.DEBUG_COLORS = false
    this.debugCompat = function () {
      process.stderr.write = this[symbol.log].write.bind(this[symbol.log])
    }
  }
}
  ...
  const result = (clean ? rm(dest) : Promise.resolve())
    .then(() => {
      if (this.debug.enabled && this.env('DEBUG_LOG')) {
        ...
        if (this.debugCompat !== this.constructor.prototype.debugCompat) {
          this.debugCompat()
        }
        this.debug.handle = process.stderr.write = fileLogHandler(this[symbol.log])
