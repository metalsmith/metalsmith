...
Metalsmith.prototype.run = unyield(function*(files, plugins){
  ...
  let run = thunkify(ware.run.bind(ware));
  let res = yield run(files, this);
  return res[0];
});
...
Metalsmith.prototype.read = unyield(function*(dir){
  ...
  let paths = yield readdir(dir, ignores);
  let files = [];
  let complete = 0;
  let batch;
...
Metalsmith.prototype.readFile = unyield(function*(file){
  ...
  try {
    ...
    let buffer = yield fs.readFile(file);
    let parsed;
