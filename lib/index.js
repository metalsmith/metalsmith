 ...
 */
/**
 * Normalize watch paths for chokidar 4 compatibility.
 * Chokidar 4 dropped glob/anymatch support, so glob patterns must be
 * expanded to their base directories, and negation patterns must be
 * moved to chokidar's `ignored` option.
 * @param {string[]} paths
 * @returns {{ paths: string[], ignored: string[] }}
 */
function normalizeWatchPaths(paths) {
  ...
  for (const p of paths) {
    const info = scan(p)
    if (info.negated) {
      ignored.push(info.base)
    } else {
      normalized.push(info.base)
    }
  }

  // deduplicate: remove paths that are subdirectories of other watched paths
  const deduplicated = [...new Set(normalized)].filter((p, _i, all) => {
    return !all.some((other) => other !== p && p.startsWith(other + path.sep))
  })

  return { paths: deduplicated, ignored }
}
...
Metalsmith.prototype.watch = function (options) {
  ...
  if (!options) {
    ...
    // wasWatching distinguishes "stop a watcher that failed to set up" (return Promise)
    // from "configure watch to off" (fall through to return this)
    ...
    // ensure paths is always an array, defaulting to source directory
    if (!options.paths) options.paths = [this.source()]
    else if (isString(options.paths)) options.paths = [options.paths]

    // normalize globs and negation patterns for chokidar 4 compatibility
    const { paths: watchPaths, ignored: negatedPaths } = normalizeWatchPaths(options.paths)
    options.paths = watchPaths
    ...
    this[symbol.watch] = {
      ...
      alwaysStat: false,
      cwd: this.directory(),
      ignored: [...this.ignore(), ...negatedPaths],
      ignoreInitial: true
    }
  }
  return this
...
Metalsmith.prototype.imports = function (specifier, namedExport) {
  ...
  return load(specifier)
    ...
    .then((s) => {
      ...
      if (arguments.length === 1) {
        const namedExports = Object.keys(s).filter((d) => d !== 'default' && d !== 'module.exports')
