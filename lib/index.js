...
function Metalsmith(directory) {
  ...
  Object.defineProperty(this, symbol.static, {
    value: {
      spec: [],
      resolution: {}
    },
      ...
      return new Promise((resolve, reject) => {
        this.process((err, files) => {
          ...
          return (clean ? rm(dest) : Promise.resolve())
            ...
            .then(() => Promise.all(Object.keys(this[symbol.static].resolution).map(staticPath => {
              const src = path.join(this.source(), staticPath)
              const target = path.join(dest, staticPath)
              return mkdir(path.dirname(target), { recursive: true })
...
Metalsmith.prototype.read = function (dir, callback) {
  ...
  const result = readdir(dir, { ignores, root: dir }).then((files) => {
     ...
     **/
    const expandedPatterns = this.statik().reduce((all, pattern) => {
      all.push(pattern)
    ...
    for (const [p, stats] of files) {
      if (staticPaths.includes(p)) statik[p] = { stats, contents: Buffer.from(p) }
      else {
        nonStatik.push([p, stats])
      }
    }
    this[symbol.static].resolution = statik
...
Metalsmith.prototype.readFile = function (file, callback) {
  ...
  const result = Promise.all([
    stats instanceof Stats ? Promise.resolve(stats) : stat(file),
    readFile(file)
  ])
    .then(([stats, buffer]) => {
      let ret = {}
...
Metalsmith.prototype.imports = function (specifier, name) {
  ...
  return load(specifier)
    ...
    .then((s) => {
      ...
      throw new Error(`Metalsmith#imports() could not load "${name}" from module "${specifier}`)
    })
}
